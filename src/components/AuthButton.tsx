...
export default function AuthButton() {
  const { user, loading, signInWithGoogle, signInWithGitHub, signOut } = useAuthStore()
  ...
  const initials = (user.email ?? user.user_metadata?.name ?? 'U')[0].toUpperCase()
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-text-secondary font-display text-xs uppercase tracking-wider hidden sm:block max-w-[120px] truncate">
        {displayName}
      </span>
      <button
        onClick={signOut}
